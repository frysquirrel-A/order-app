...
const insertInitialData = async () => {
  try {
    // 기존 데이터 확인
    const menuCheck = await query('SELECT COUNT(*) FROM menus')
    if (parseInt(menuCheck.rows[0].count) > 0) {
      console.log('초기 데이터가 이미 존재합니다. 건너뜁니다.')
      return
    }
...
// 스크립트 직접 실행 시
if (import.meta.url.endsWith(process.argv[1].replace(/\\/g, '/')) || process.argv[1].includes('init-db.js')) {
  init()
}

export { createSchema, insertInitialData }
